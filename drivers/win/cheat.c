...
static HWND hwndLB;
int RedoCheatsCallB(char *name, uint32 a, uint8 v, int s) {
	char str[259] = { 0 };
	strcpy(str,(s?"* ":"  "));
	strcat(str,name);
			...
			switch (HIWORD(wParam)) {
				case BN_CLICKED:
					switch (LOWORD(wParam)) {
						...
						case 303: //Update
							...
							FCEUI_SetCheat(selcheat,str,a,v,-1);
							FCEUI_GetCheat(selcheat,&name,&a,&v,&s);
							strcpy(str2,(s?"* ":"  "));
							strcat(str2,str);
				...
				case LBN_DBLCLK:
					switch (LOWORD(wParam)) {
						case 101:
							FCEUI_GetCheat(selcheat,&name,&a,&v,&s);
							FCEUI_SetCheat(selcheat,0,-1,-1,s^=1);
							strcpy(str,(s?"* ":"  "));
							strcat(str,name);
							SendDlgItemMessage(hwndDlg,101,LB_DELETESTRING,selcheat,0);
...
void ConfigCheats(HWND hParent) {
	...
	if (!CheatWindow) {
		selcheat=-1;
		CheatWindow=1;
		if (CheatStyle) CreateDialog(fceu_hInstance,"CHEATCONSOLE",NULL,CheatConsoleCallB);
